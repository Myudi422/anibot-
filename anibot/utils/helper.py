...
def get_btns(media, user: int, result: list, lsqry: str = None, lspage: int = None, auth: bool = False, sfw: str = "False"):
    ...
    if media == "CHARACTER":
        buttons.append([InlineKeyboardButton("⌥ Deskripsi", url=f"https://t.me/ccgnimeX_bot/?start=des_{result[2][0]}_CHAR{qry}{pg}_{str(auth)}_{user}")])
...
def get_auth_btns(media, user, data, name, lsqry: str = None, lspage: int = None):
    ...
    if media=="CHARACTER":
        ...
        if media=="ANIME":
            btn.append(InlineKeyboardButton("〧 Unduh", switch_inline_query_current_chat=name))
